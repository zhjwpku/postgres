...
DROP USER IF EXISTS graph_rls_alice;
DROP USER IF EXISTS graph_rls_bob;
DROP USER IF EXISTS graph_rls_carol;
DROP USER IF EXISTS graph_rls_dave;
DROP USER IF EXISTS graph_rls_exempt_user;
DROP ROLE IF EXISTS graph_rls_group1;
DROP ROLE IF EXISTS graph_rls_group2;
...
GRANT graph_rls_group1 TO graph_rls_dave;
GRANT graph_rls_group2 TO graph_rls_bob;
...
CREATE POLICY p2 ON document_people AS RESTRICTIVE TO graph_rls_dave
    USING (dtitle <> 'Scientists');
CREATE POLICY p3 ON document_people AS RESTRICTIVE TO graph_rls_group1
    USING (dlevel < 3);
CREATE POLICY p4 ON document_people AS PERMISSIVE TO graph_rls_group2
    USING (dlevel < 3);
...
-- viewpoint from graph_rls_bob
SET SESSION AUTHORIZATION graph_rls_bob;
EXECUTE graph_rls_query;

-- viewpoint from graph_rls_carol
SET SESSION AUTHORIZATION graph_rls_carol;
EXECUTE graph_rls_query;

-- viewpoint from graph_rls_dave
SET SESSION AUTHORIZATION graph_rls_dave;
EXECUTE graph_rls_query;
...
SET SESSION AUTHORIZATION graph_rls_exempt_user;
EXECUTE graph_rls_query;
...
CREATE POLICY p1 ON document AS PERMISSIVE
    USING (dlevel <= (SELECT seclv FROM users WHERE pguser = current_user));
CREATE POLICY p2 ON document AS RESTRICTIVE TO graph_rls_dave
    USING (dtitle <> 'Scientists');
...
CREATE POLICY pr ON document TO graph_rls_dave
