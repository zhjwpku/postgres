...
typedef struct RI_ConstraintInfo
{
	...
	bool		fpmeta_valid; /* Fast-path RI check */
} RI_ConstraintInfo;
...
static bool
ri_fastpath_stmt_ensure(const RI_ConstraintInfo *riinfo, Relation pk_rel)
{
	/*
	...
	/*
	 * Handle multi-column keys later.
	 */
	if (riinfo->nkeys > 1)
		return false;
...
RI_FKey_check(TriggerData *trigdata)
{
	...
	if (riinfo->fpmeta_valid &&
		ri_fastpath_stmt_ensure(riinfo, pk_rel))
	{
		Oid			idxoid;
		Relation	idxrel;
		int			num_pk;
		 ...
		 *
		 * Handle partitioned 'pk_rel' later, skipped in ri_fastpath_stmt_ensure
		 */
		...
		if (!index_getnext_tid(scan, ForwardScanDirection))
		...
		tm = table_tuple_lock(pk_rel, &scan->xs_heaptid,
							  GetActiveSnapshot(),
							  outslot,
		...
		/* Fall back to SPI */
		elog(DEBUG1, "RI fastpath: constraint \"%s\" falling back to SPI (%u)",
			 NameStr(riinfo->conname), tm);
	}
