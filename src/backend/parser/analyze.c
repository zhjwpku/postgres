 ...
 * common type, typmod, and collation representing the output (UNION) column. If
 * `targetlist` is not NULL, also build the dummy output targetlist containing
 * non-resjunk output columns. The values are stored into the given
