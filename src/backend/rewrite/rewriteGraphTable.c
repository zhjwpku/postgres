...
/*
 * Return a list of all the graph elements that satisfy the graph element
 * pattern represented by the given path_factor `pf`.
 *
 * First we find all the graph labels that satisfy the label expression in
 * path factor. Each label is associated with one or more graph elements.
 * A union of all such elements satisfies the element pattern. The returned
 * list contains one path_element object representing each of these
